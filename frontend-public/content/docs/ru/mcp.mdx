...
| `payable_network` | string | Да | Сеть оплаты подписки recv. Одно из: `TON`, `TRON`, `BASE`, `BSC`. Используйте `payable_asset` или `payment_options`, чтобы выбрать `TON`/`GRAM` или `TON`/`USDT`. |
...
| `payable_network` | string | Да | Сеть оплаты покупателя. Одно из: `TON`, `TRON`, `BASE`, `BSC`. Используйте `payable_asset` или `payment_options`, чтобы выбрать `TON`/`GRAM` или `TON`/`USDT`. |
...
Список поддерживаемых вариантов оплаты. Не принимает параметров и возвращает статичную сводку пар сеть/актив (`TON`/`GRAM`, `TON`/`USDT`, `TRON`/`USDT`, `BASE`/`USDT`, `BSC`/`USDT`). См. [Поддерживаемые сети](/ru/docs/networks) для авторитетного списка.
