...
| `payable_network` | string | Да | Вариант оплаты подписки recv. Одно из: `TON` (GRAM в TON), `TON_USDT` (USDT в TON), `TRON`, `BASE`, `BSC`. |
...
| `payable_network` | string | Да | Вариант оплаты покупателя. Одно из: `TON` (GRAM в TON), `TON_USDT` (USDT в TON), `TRON`, `BASE`, `BSC`. |
...
Список поддерживаемых вариантов оплаты. Не принимает параметров и возвращает статичную сводку значений `payable_network` (`TON` для GRAM в TON, `TON_USDT` для USDT в TON, `TRON`, `BASE`, `BSC`). См. [Поддерживаемые сети](/ru/docs/networks) для авторитетного списка.
