...
Whenever an event is triggered, Reqst sends a `POST` request to your endpoint with the following headers:

- `X-Reqst-Event`: The type of event (e.g., `invoice.paid`).
- `X-Reqst-Timestamp`: The Unix epoch timestamp when the webhook was sent (in seconds).
- `X-Reqst-Signature`: The signature of the request (prefixed with `v1=`).
...
To ensure that a webhook was actually sent by Reqst, you **must** verify the `X-Reqst-Signature` header using your **Webhook Secret** as the key.
...
app.post('/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  const secret = process.env.REQST_WEBHOOK_SECRET;
...
@app.post("/webhook")
def webhook():
    signature = request.headers.get("X-Reqst-Signature", "")
    timestamp = request.headers.get("X-Reqst-Timestamp", "")
...
| `subscription.activated` | A Reqst plan/subscription invoice settled (fired alongside `invoice.paid`). |

## Retries

Reqst retries failed deliveries (non-2xx response, timeout, or connection error). The maximum number of attempts is set by your plan's webhook retry budget — **3** on Developer, **5** on Business, **8** on Enterprise. Deliveries and their attempts are visible in the Developer Portal, where you can also resend manually.

## Best Practices

- **Respond with 200 OK**: Always return a 200 status code to acknowledge receipt of the webhook.
- **Idempotency**: Your webhook handler should be idempotent, as Reqst may retry the same notification if it doesn't receive a 200 response. Dedupe on `transition_id` (or `invoice.public_id` + `status`).
