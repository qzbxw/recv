...
| `payable_network` | string | Yes | Network used to pay recv. One of: `TON`, `TRON`, `SOLANA`, `BASE`, `ARBITRUM`, `BSC`. Use `payable_asset` or `payment_options` to choose assets like `TON`/`GRAM` vs `TON`/`USDT`. |
...
| `payable_network` | string | Yes | Network the customer pays on. One of: `TON`, `TRON`, `SOLANA`, `BASE`, `ARBITRUM`, `BSC`. Use `payable_asset` or `payment_options` to choose assets like `TON`/`GRAM` vs `TON`/`USDT`. |
...
List the payment options recv supports. Takes no parameters and returns a static summary of network/asset pairs (`TON`/`GRAM`, `TON`/`USDT`, `TRON`/`USDT`, `SOLANA`/`SOL`, `SOLANA`/`USDT`, `SOLANA`/`USDC`, `BASE`/`USDT`, `BASE`/`USDC`, `ARBITRUM`/`USDT`, `ARBITRUM`/`USDC`, `BSC`/`BNB`, `BSC`/`USDT`). See [Supported Networks](/en/docs/networks) for the authoritative list.
