...
| `payable_network` | string | Yes | Network used to pay recv. One of: `TON`, `TRON`, `BASE`, `BSC`. Use `payable_asset` or `payment_options` to choose `TON`/`GRAM` vs `TON`/`USDT`. |
...
| `payable_network` | string | Yes | Network the customer pays on. One of: `TON`, `TRON`, `BASE`, `BSC`. Use `payable_asset` or `payment_options` to choose `TON`/`GRAM` vs `TON`/`USDT`. |
...
List the payment options recv supports. Takes no parameters and returns a static summary of network/asset pairs (`TON`/`GRAM`, `TON`/`USDT`, `TRON`/`USDT`, `BASE`/`USDT`, `BSC`/`USDT`). See [Supported Networks](/en/docs/networks) for the authoritative list.
