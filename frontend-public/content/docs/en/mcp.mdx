...
The server is `recv-agent-mcp` and runs locally over stdio. Under the hood it calls the same recv API documented elsewhere in these docs. It can also bootstrap a new agent workspace, create a recv subscription checkout, and issue API credentials after the subscription is paid.
...
Create a new trial workspace for a fully autonomous agent. The tool returns a normal recv console auth result; persist `token` or `access_token` as `RECV_ACCESS_TOKEN` before calling self-service tools.
...
| `payable_network` | string | Yes | Network used to pay recv. One of: `TON`, `TON_USDT`, `TRON`, `BASE`, `BSC`. |
...
| `payable_network` | string | Yes | Network the customer pays on. One of: `TON`, `TON_USDT`, `TRON`, `BASE`, `BSC`. |
...
List the blockchain networks recv supports. Takes no parameters and returns a static summary of the `payable_network` values (TON, TON_USDT, TRON, BASE, BSC). See [Supported Networks](/en/docs/networks) for the authoritative list.
...
1. Call `bootstrap_agent_workspace` and store the returned access token as `RECV_ACCESS_TOKEN`.
...
4. Call `create_api_key` and store the returned `secret` as `RECV_API_KEY`.
5. Optionally call `create_webhook_endpoint` and store the returned webhook secret as `RECV_WEBHOOK_SECRET`.
