...
describe("bilingual fallback articles", () => {
  it("contains eight reciprocal EN/RU article pairs", () => {
    expect(FALLBACK_BLOG_POSTS.en).toHaveLength(8);
    expect(FALLBACK_BLOG_POSTS.ru).toHaveLength(8);
  ...
  it("has publish-ready metadata and internal links", () => {
    for (const locale of ["en", "ru"] as const) {
      for (const post of FALLBACK_BLOG_POSTS[locale]) {
        ...
        expect(post.content_md.match(/\]\(\/(?:en|ru)\//g)?.length).toBeGreaterThanOrEqual(2);
